...
class EnseignantAuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        $enseignant = Enseignant::where('numero_enseignant', $request->numero_enseignant)->first();
        ...
        $token = $enseignant->createToken('enseignant-mobile')->plainTextToken;

        return response()->json([
            'token'       => $token,
            'enseignant'  => [
                'id'       => $enseignant->id,
                'numero'   => $enseignant->numero_enseignant,
