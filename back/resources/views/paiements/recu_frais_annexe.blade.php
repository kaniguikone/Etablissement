...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .categorie-badge {
        ...
        background: #e8f4fd; color: #1a5276; margin-bottom: 12px;
    }
    ...
    .solde-cell .val { font-size: 15px; font-weight: bold; margin-top: 3px; }
    .solde-cell .val.solde-zero { color: #27ae60; }
    .solde-cell .val.solde-reste { color: #e67e22; }
    ...
    .signature-bloc { margin-top: 30px; display: table; width: 100%; font-size: 11px; }
...
@php
    ...
    // Calculer le total payé pour ce frais par cet élève (toutes transactions)
    $totalPayeGlobal = \App\Models\PaiementFraisAnnexe::where('frais_annexe_id', $frais->id)
        ->where('eleve_id', $eleve->id)->sum('montant_paye');
    $solde = max(0, (float) $frais->montant - (float) $totalPayeGlobal);
@endphp
...
<div class="titre-recu">Reçu de frais annexe</div>

<div class="numero">N° FA-{{ str_pad($paiement->id, 6, '0', STR_PAD_LEFT) }} — Émis le {{ now()->format('d/m/Y') }}</div>

<div style="text-align:center">
    <span class="categorie-badge">{{ $categories[$frais->categorie] ?? $frais->categorie }}</span>
</div>
...
<div class="bloc">
    <h3>Détails du frais</h3>
    <table class="info">
        <tr>
            <td class="lbl">Désignation :</td>
            <td><strong>{{ $frais->nom }}</strong></td>
        </tr>
        ...
        @endif
        <tr>
            <td class="lbl">Montant total dû :</td>
            <td>{{ number_format($frais->montant, 0, ',', ' ') }} FCFA</td>
        </tr>
        <tr>
...
<div class="montant-bloc">
    <div class="label">Montant reçu ce jour</div>
...
<div class="solde-bloc">
    <div class="solde-cell">
        <div style="color:#555">Total dû</div>
        <div class="val">{{ number_format($frais->montant, 0, ',', ' ') }} FCFA</div>
    </div>
    <div class="solde-cell">
        <div style="color:#555">Total payé</div>
        <div class="val">{{ number_format($totalPayeGlobal, 0, ',', ' ') }} FCFA</div>
    </div>
    <div class="solde-cell">
        <div style="color:#555">Solde restant</div>
        <div class="val {{ $solde <= 0 ? 'solde-zero' : 'solde-reste' }}">
            {{ number_format($solde, 0, ',', ' ') }} FCFA
        </div>
    </div>
</div>

<div class="watermark">{{ $solde <= 0 ? 'Soldé' : 'Partiel' }}</div>
