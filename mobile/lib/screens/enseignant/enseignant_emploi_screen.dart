...
class EnseignantEmploiScreen extends StatefulWidget {
  const EnseignantEmploiScreen({super.key});

  @override
  State<EnseignantEmploiScreen> createState() => _EnseignantEmploiScreenState();
}

class _EnseignantEmploiScreenState extends State<EnseignantEmploiScreen>
    with SingleTickerProviderStateMixin {
  final _api = ApiService();
  Map<String, List<Creneau>> _emploi = {};
  Map<String, Color> _couleurs = {};
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await _api.getEmploiDuTempsEnseignant();
      final Map<String, List<Creneau>> result = {};
      data.forEach((jour, liste) {
        result[jour] = (liste as List)
      ...
      for (final creneaux in result.values) {
        for (final c in creneaux) {
          if (c.matiereAbbr != null && !pal.containsKey(c.matiereAbbr)) {
            pal[c.matiereAbbr!] = _palette[idx % _palette.length];
            idx++;
          }
        }
      }

      setState(() { _emploi = result; _couleurs = pal; _loading = false; });
    } catch (e) {
                  ...
                  : TabBarView(
                      ...
                      children: _jours.map((jour) {
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            final c = creneaux[i];
                            final couleur = _couleurs[c.matiereAbbr] ?? AppTheme.primary;
                            return _CreneauEnseignantCard(creneau: c, couleur: couleur);
                          },
                        );
                      }).toList(),
                    ),
...
class _CreneauEnseignantCard extends StatelessWidget {
  final Creneau creneau;
  final Color couleur;

  const _CreneauEnseignantCard({required this.creneau, required this.couleur});
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (creneau.classe != null) ...[
                      const SizedBox(height: 3),
                      Row(
                        children: [
                          Icon(Icons.class_outlined, size: 13, color: couleur),
                          const SizedBox(width: 4),
                          Text(creneau.classe!,
