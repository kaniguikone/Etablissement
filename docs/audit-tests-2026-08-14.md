...
> **Mise à jour (2026-08-16)** : les 14 correctifs listés ci-dessous ont été appliqués et validés, y compris le test réel sur émulateur Android et la Phase 5 (dette technique) — voir [Statut final des correctifs](#statut-final-des-correctifs) en fin de document. Le corps de l'audit ci-dessous est conservé tel qu'écrit initialement, à titre de référence historique (les chiffres qui y figurent — 86 tests, bundle 1 Mo, 583 erreurs ESLint — sont ceux du 14/08, avant correctifs).
...
- **Unicité `matricule_enseignant`** : contrainte `unique()` ajoutée dans la migration `create_enseignants_table` (tenant), et validation Laravel correspondante dans `EnseignantController::store`/`update`.
  **✅ Appliquée** sur les 4 bases tenant locales existantes (`benedictions-college`, `benedictions-lycee`, `college-baraka`, `lycee-douahou`) après vérification qu'aucun doublon n'existait, via `ALTER TABLE enseignants ADD UNIQUE (matricule_enseignant)`. **Reste à faire en production** : appliquer la même vérification + `ALTER TABLE` sur chaque base tenant de production avant déploiement (la migration seule ne s'applique pas rétroactivement aux bases déjà provisionnées).
...
### Phase 4 — Mobile (✅ terminée et testée sur émulateur Android 11+)
...
- **✅ Testé** sur émulateur Android 17 (API 37, Pixel 8a) : connexion parent, ouverture d'un reçu de paiement PDF depuis l'écran Scolarités. Le sélecteur "Ouvrir avec" liste correctement Chrome et Drive PDF Viewer (confirmé au niveau système via les logs `ActivityTaskManager`) — c'est exactement le mécanisme de visibilité de paquets Android 11+ que le bloc `<queries>` corrige. Bug supplémentaire trouvé et corrigé pendant ce test : `switchEcole()` (connexion parent/enseignant unifiée, `mobile/lib/services/api_service.dart`) ne résolvait pas le domaine tenant renvoyé par l'API de la même façon que le reste de l'app, cassant tout test local sur émulateur (fonctionnait en production où le domaine est un vrai nom DNS).

### Phase 5 — Dette technique (✅ terminée)
- **Code-splitting frontend** : les ~120 pages de `RoutesMenu.jsx` étaient importées statiquement, produisant un bundle unique de 1064 Ko. Converties en `React.lazy()` + `Suspense` : le chunk principal passe à 260 Ko, chaque page se charge à la demande.
- **Tests mobile** : le seul test présent (`widget_test.dart`) était le boilerplate Flutter par défaut, jamais adapté, qui plantait faute des `Provider` requis par l'app. Remplacé par un smoke test fonctionnel sur `OnboardingScreen`.
- **Dette ESLint** : 701 → 694 problèmes. Un vrai bug corrigé au passage (`EnseignantRdv.jsx` appelait une fonction `confirmer` jamais importée — l'annulation d'un RDV enseignant plantait), et un fichier mort supprimé (`enseignants/Ense.jsx`, fragment JSX non fonctionnel, non référencé nulle part). Le gros de la dette restante (576 erreurs) est du `react/prop-types` (376, cosmétique) et `react-hooks/exhaustive-deps` (111, risqué à corriger en masse sans revue au cas par cas) — non traité.
...
- **Déploiement de l'instance démo publique** (VPS + DNS wildcard) et **`Fonctionnalités.docx`** (fichier binaire obsolète, probablement à supprimer) : hors du périmètre technique de cette série de correctifs, nécessitent une décision/action de votre part.
