...
> - **Lot 4 — Tableau de bord établissement** : ✅ livré. Endpoint `/budget/dashboard` (répartition par catégorie + historique mensuel) + écran `BudgetTableauBord.jsx`. Exports (2026-09-12) : `GET /budget/export-excel` (registre des dépenses, `PhpSpreadsheet`, même pattern que `StatsGeneralesController`) et `GET /budget/rapport-pdf` (rapport imprimable — dotations approuvées, répartition par catégorie, détail des dépenses — `resources/views/budget/rapport.blade.php`, même pattern que `FraisAnnexeController::recu`), boutons dans `BudgetTableauBord.jsx`. Vérifiés en conditions réelles (téléchargement déclenché au clic, fichiers valides). **Non livré : email de relance DG** — reporté faute de besoin confirmé à l'usage (le pull via le dashboard groupe suffit pour l'instant, cf. §8).
...
| `GET /budget/export-excel` | `budget_gestion` ou `budget_validation` | Registre des dépenses de l'année (`.xlsx`), pattern `StatsGeneralesController::exportExcel`. |
| `GET /budget/rapport-pdf` | `budget_gestion` ou `budget_validation` | Rapport imprimable — dotations approuvées, répartition par catégorie, détail des dépenses (`.pdf`), pattern `FraisAnnexeController::recu`. |
