...
> - **Lot 4 — Cas avancés** : ✅ livré (branche `feat/edt-lot4-groupes`). Table `groupes_pedagogiques` (LV2 / dédoublements) + `emploi_du_temps.groupe_id` / `semaine` (A/B). Le `Generateur` a été refondu autour de la notion de « sous-cours » : un besoin porte 1 sous-cours (cas général) ou N (groupes parallèles placés sur le même créneau, salles/profs différents). Quinzaine : alternance A/B par classe (PC en A, SVT en B → même créneau). Séances 1h30 : `nb_plages` calculé sur la durée médiane réelle de la grille. `Validateur` : nouvelle règle `GROUPES_PARALLELES` (D15), collisions désormais conscientes de la semaine et des groupes. Écran `/GroupesPedagogiques` (Paramétrage), diagnostic « groupes sans enseignant ». **Différé : phasage explicite du solveur, souples S3/S4/S6/S8/S10, tandem PC/SVT strictement contigu sans récré (seul « même jour » est vérifié).**
> - **Lot 5 — Multi-sites & finitions** : non prévu pour l'instant. Concerne un établissement *unique* réparti sur plusieurs bâtiments ; un groupe scolaire = des tenants séparés, déjà gérés indépendamment. À reprendre si un tenant multi-campus apparaît.
> - **Réorganisation du menu EDT** : ✅ livré (branche `feat/edt-menu`). Nouveau groupe latéral « Emploi du temps » regroupant grille horaire, groupes pédagogiques, indisponibilités, diagnostic, génération, emplois du temps, contrôle, conformité. Regroupement **d'affichage uniquement** : les entrées gardent leur slug de module d'origine, les routes et le gating par établissement sont inchangés.
